...
from .serializer import attendence,uploded_video
...
class All_sample(viewsets.ModelViewSet):
    permission_classes = [BlocklistPermission]
    queryset = Attendence_sample_video.objects.all()
    serializer_class = (uploded_video)
...
def upload_face_video(request):
    ...
    if snippet.exists():
        ...
        if snippet.sample_taken == False:
            ...
            # file = fs.save("abc.mp4", request.data["file"])
            data["file"] =  base64_file(request.data["file"],"video")
            data["employee_id"] = snippet.id

            serializer1 = uploded_video(data=data)
            if serializer1.is_valid(request.data):
                serializer1.save()

    return Response(
        { "status": status.HTTP_200_OK, "data": serializer1.data})
    # except:
...
def create_dataset(request):
    ...
    if snippet.exists():
        ...
        if  snippet.sample_taken==False:
            ...
            ret, frame = cv2.VideoCapture(0).read()
            cam = cv2.VideoCapture(0)
            faceDetect = cv2.CascadeClassifier(str(os.path.join(BASE_DIR))+'/ml/haarcascade_frontalface_default.xml')
            ...
            while(True):
                ...
                for (x, y, w, h) in faces:
                    ...
                    cv2.imwrite(str(os.path.join(BASE_DIR))+'/ml/dataset/user.' + str(id) + '.' + str(sampleNum) + '.jpg',gray[y:y + h, x:x + w])
...
def trainer(request):
    ...
    #Path of the samples
    path = str(os.path.join(BASE_DIR))+'/ml/dataset'
    ...
    recognizer.save(str(os.path.join(BASE_DIR))+'/ml/recognizer/trainingData.yml')
    cv2.destroyAllWindows()
...
def detect(request):
    ...
    rec.read(str(os.path.join(BASE_DIR))+'/ml/recognizer/trainingData.yml')
    getId = 0
...
@api_view(['GET'])
def create_dataset_new(request):
    #print request.POST "DEMO25/004"
    employee_id = request.data['employee_id']
    snippet = Employeess.objects.filter(employee_id=employee_id)
    if snippet.exists():
        snippet = Employeess.objects.get(employee_id=employee_id)
        if  snippet.sample_taken==False:
            ...
            # ret, frame = cv2.VideoCapture(0).read()
            data={}
            data["file"]=file=request.data["file"]
            data["employee_id"] = snippet.id
            ...
            cam = cv2.VideoCapture(str(os.path.join(BASE_DIR)) + "/media/facesamplecollection/"+str(snippet.id)+"_.mp4")
            if not cam.isOpened():
                print("False")
            faceDetect = cv2.CascadeClassifier(str(os.path.join(BASE_DIR))+'/ml/haarcascade_frontalface_default.xml')
            # cap = cv2.VideoCapture(1)
